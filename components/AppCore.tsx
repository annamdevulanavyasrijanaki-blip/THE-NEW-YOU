...
export default function AppCore() {
  ...
  const handleTryOnFromShop = async (dressImage: string, dressName: string) => {
    ...
    try {
        ...
        console.error("Critical Asset Sync Failure", e);
        alert("Atelier recalibration required. Please select an alternative garment.");
    } finally {
  ...
  const handleSaveLook = async (look: SavedLook) => {
      ...
      try {
          ...
          if (isDuplicate) {
            alert("This silhouette already exists in your digital archive.");
          ...
          alert("Archive established. ✨");
      } catch (err) {
          console.error("Sync Failure", err);
          alert("Archive synchronization interrupted.");
      } finally {
          setIsUploading(false);
      }
  };
  ...
  if (isAuthChecking) {
    return <PageContainer><LoadingOverlay message="Authenticating Connection..." /></PageContainer>;
  }
